  ...
  <div>
    <div class="avatar cursor-pointer " @mouseover="isHover=true" @mouseout="isHover=false" @click="openChatField">
      <div class=" w-60 h-100 rounded-2xl relative " >
        <img :src="character.background_image" alt="" class="transition-transform duration-300" :class="{'scale-120':isHover}">
        <div class="absolute left-0 top-50 w-60 h-50 bg-linear-to-t from-black/40 to-transparent"></div>
        <div v-if="canEdit && character.author.user_id === user.id " class="absolute right-0 top-60">
          <RouterLink @click.stop :to="{name:'update-page',params:{character_id:character.id}}" class="btn btn-circle btn-ghost bg-transparent">
            <UpdateIcon/>
          </RouterLink>
        ...
        <div class="absolute left-4 top-64 avatar">
          <div class="  w-16 rounded-full ring-3 ring-white">
            <img :src="character.photo" alt="">
          </div>
        </div>
        <div class="absolute left-24 top-68 right-4 text-white font-bold line-clamp-1 break-all">
          {{character.name}}
        </div>
        <div class="absolute left-5 top-82 right-4 text-sm text-white line-clamp-3 break-all">
          {{character.profile}}
        </div>
      </div>
    </div>
    <RouterLink :to="{name:'user-space-page',params:{user_id: character.author.user_id}}" class="flex items-center mt-4 gap-2 w-60">
      <div class="avatar">
