...
<template>
  <div class="flex justify-center mt-30">
    <form @submit.prevent="handleRegister" class="fieldset bg-base-200 border-base-300 rounded-box w-xs border p-4">
      ...
      <a v-if="errorMessage" class="text-sm text-red-500 mt-1">{{errorMessage}}</a>

      <button class="btn btn-neutral mt-4">注册</button>
      <div class="flex justify-end">
        <RouterLink :to="{name:'user-account-login-page'}" class="btn btn-sm btn-ghost text-gray-500">
          登录
        </RouterLink>
      </div>
    </form>
